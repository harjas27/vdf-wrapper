package wrapper

import (
	"crypto/rand"
	"encoding/hex"
	"fmt"
	"github.com/harmony-one/vdf/src/vdf_go"
	"github.com/stretchr/testify/assert"
	"log"
	"testing"
	"time"
)

func TestGenerateVDFAndVerify(t *testing.T) {
	input :=  [32]byte{0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe,
		0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,}
	vdf := New(10000, input)

	outputChannel := vdf.GetOutputChannel()
	start := time.Now()

	vdf.Execute()

	duration := time.Now().Sub(start)


	output := <-outputChannel

	fmt.Println(fmt.Sprintf("VDF computation finished, result is  %s", hex.EncodeToString(output[:])))
	fmt.Println(fmt.Sprintf("VDF computation finished, time spent %s", duration.String()))
	assert.Equal(t, true, vdf.Verify(output), "failed verifying proof")
}

func TestGenerateVDFAndVerifyUsingGoVDF(t *testing.T) {
	input :=  [32]byte{0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe,
		0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,}
	vdf := vdf_go.New(10000, input)

	outputChannel := vdf.GetOutputChannel()
	start := time.Now()

	vdf.Execute()

	duration := time.Now().Sub(start)


	output := <-outputChannel

	fmt.Println(fmt.Sprintf("VDF computation finished, result is  %s", hex.EncodeToString(output[:])))
	fmt.Println(fmt.Sprintf("VDF computation finished, time spent %s", duration.String()))
	assert.Equal(t, true, vdf.Verify(output), "failed verifying proof")
}

func TestVerifyVDF(t *testing.T) {
	input :=  [32]byte{0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe,
		0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,}
	inputVDF,_ := hex.DecodeString("")

	var vdfBytes [516]byte
	copy(vdfBytes[:], inputVDF)

	vdf := New(100, input)

	start := time.Now()

	result := vdf.Verify(vdfBytes)

	duration := time.Now().Sub(start)

	log.Println(fmt.Sprintf("VDF verification finished, time spent %s", duration.String()))
	assert.Equal(t, true, result, "failed verifying vdf proof")
}

func TestVerifyVDFUsingGoVDF(t *testing.T) {
	input :=  [32]byte{0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe,
		0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,0xde, 0xad, 0xbe, 0xef,}
	inputVDF,_ := hex.DecodeString("")

	var vdfBytes [516]byte
	copy(vdfBytes[:], inputVDF)

	vdf := vdf_go.New(100, input)

	start := time.Now()

	result := vdf.Verify(vdfBytes)

	duration := time.Now().Sub(start)

	log.Println(fmt.Sprintf("VDF verification finished, time spent %s", duration.String()))
	assert.Equal(t, true, result, "failed verifying vdf proof")
}

func TestVDFModuleRandomSeed(t *testing.T) {

	input := [32]byte{}
	rand.Read(input[:])

	vdf := New(10000, input)

	outputChannel := vdf.GetOutputChannel()

	start := time.Now()

	vdf.Execute()

	duration := time.Now().Sub(start)

	log.Println(fmt.Sprintf("VDF computation finished, time spent %s", duration.String()))

	output := <-outputChannel

	assert.Equal(t, true, vdf.Verify(output), "failed verifying proof")

}

func TestVDFModuleRandomSeedUsingGoVDF(t *testing.T) {

	input := [32]byte{}
	rand.Read(input[:])

	vdf := vdf_go.New(10000, input)

	outputChannel := vdf.GetOutputChannel()

	start := time.Now()

	vdf.Execute()

	duration := time.Now().Sub(start)

	log.Println(fmt.Sprintf("VDF computation finished, time spent %s", duration.String()))

	output := <-outputChannel

	assert.Equal(t, true, vdf.Verify(output), "failed verifying proof")

}

func benchmarkVDFRustGenerateAndVerify(difficulty int, b *testing.B) {
	bytes := [32]byte{0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe,
		0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vdf := New(difficulty, bytes)
		vdf.Execute()
		vdf.Verify(vdf.output)
	}
}

func benchmarkVDFGoGenerateAndVerify(difficulty int, b *testing.B) {
	bytes := [32]byte{0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe,
		0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vdf := vdf_go.New(difficulty, bytes)
		vdf.Execute()
		vdf.Verify(vdf.GetOutput())
	}
}

func BenchmarkVDFRustGenerateAndVerify100(b *testing.B) {
	benchmarkVDFRustGenerateAndVerify(100, b)
}

func BenchmarkVDFGoGenerateAndVerify100(b *testing.B) {
	benchmarkVDFGoGenerateAndVerify(100, b)
}

func BenchmarkVDFRustGenerateAndVerify1000(b *testing.B) {
	benchmarkVDFRustGenerateAndVerify(1000, b)
}

func BenchmarkVDFGoGenerateAndVerify1000(b *testing.B) {
	benchmarkVDFGoGenerateAndVerify(1000, b)
}

func BenchmarkVDFRustGenerateAndVerify10000(b *testing.B) {
	benchmarkVDFRustGenerateAndVerify(10000, b)
}

func BenchmarkVDFGoGenerateAndVerify10000(b *testing.B) {
	benchmarkVDFGoGenerateAndVerify(10000, b)
}

func BenchmarkVDFRustGenerateAndVerify100000(b *testing.B) {
	benchmarkVDFRustGenerateAndVerify(100000, b)
}

func BenchmarkVDFGoGenerateAndVerify100000(b *testing.B) {
	benchmarkVDFGoGenerateAndVerify(100000, b)
}

func BenchmarkVDFRustVerify(b *testing.B) {
	input := [32]byte{0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe,
		0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef}
	inputVDF, _ := hex.DecodeString("")

	var vdfBytes [516]byte
	copy(vdfBytes[:], inputVDF)

	vdf := New(100, input)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vdf.Verify(vdfBytes)
	}
}

func BenchmarkVDFGoVerify(b *testing.B) {
	input := [32]byte{0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe,
		0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef}
	inputVDF, _ := hex.DecodeString("")

	var vdfBytes [516]byte
	copy(vdfBytes[:], inputVDF)

	vdf := vdf_go.New(100, input)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vdf.Verify(vdfBytes)
	}
}